...
<div in:fly={{ y: 50 }} class="flex flex-col mb-3">
	<div class="flex items-center mb-4">
		...
		<h1 class="text-lg text-balance" style="color: {project.color}">{project.name}</h1>
	</div>
...
<p
	...
	class="mb-8 text-balance text-sm border-t pt-4 mt-4 border-white/10"
>
	{project.description}
</p>
...
<div
	...
	class="block relative before:inset-0 before:bg-gradient-to-r before:to-50% before:from-white/20"
>
	...
	<div class="block mb-6 text-sm">
		{#each project.features as feature}
			<span
				class="block p-5 mb-3 rounded border border-neutral-400 dark:text-neutral-200 backdrop-blur"
				>{feature}</span
			>
		{/each}
	</div>
...
<div in:fly={{ y: 50, delay: 400 }} class="block text-sm">
	...
	{#each project.contributions as contribution, index}
		<span
			in:fly|global={{ y: 50, delay: 400 + index * 100 }}
			class="block p-5 mb-3 rounded border border-neutral-400 dark:text-neutral-200"
			>{contribution}</span
		>
	{/each}
</div>
